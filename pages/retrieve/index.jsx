...
const Retrieve = () => {
  ...
  const handleSearch = (e) => {
    ...
    setFileName(filename.filter((doc) =>
      doc.name.includes(search)
    ))
    setVideoName(videoname.filter((doc) => doc.name.includes(search)))
    ...
    <div className="flex flex-col justify-center  overflow-hidden bg-blue-100">
      ...
      {si && (
        <div className="flex flex-wrap w-screen gap-3  m-5 ">
          {filename.map((doc) => {
            return (
              <div key={doc.id} className="w-[12.5rem] h-64 mb-5 bg-white  border border-black rounded-md hover:scale-[1.09] duration-300">
                <img
                  key={doc.id}
      ...
      {sv && (
        <div className="flex flex-wrap w-screen gap-5 m-5 ">
          {videoname.map((doc) => {
            return (
              <div key={doc.id} className="w-[320px] h-60  bg-white border border-black rounded-md hover:scale-[1.09] duration-300">
                <video
                  key={doc.id}
                  className=" h-44 "
                  src={doc.videourl}
